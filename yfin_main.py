...
# Загрузка данных курса доллара за последние 6 месяцев
df = yf.download('USDRUB=X', period='6mo')
...
dq.to_csv('Прогнозы 6 месяцев.csv',
          index=False, mode='a',
          header=not bool(open('Прогнозы 6 месяцев.csv').read()))
